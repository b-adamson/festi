...
for f in range(SCENE_LENGTH):
    ...
    box.insertKeyframe(f, fs.KEYFRAME.POS_ROT_SCALE)
    box.insertKeyframe(f, fs.KEYFRAME.AS_INSTANCE)
    ...
    for i in range(4):
        ...
        flames[i].insertKeyframe(f, fs.KEYFRAME.VISIBILITY)
    ...
    # floor face data
    floorData = fs.ObjFaceData()
    floorData.uvOffset = [random.random(), random.random()]
    floorData.contrast = 5.0
    floorData.materialID = fs.material("graybrick")
    floor.setFaces(floorData)
    floor.insertKeyframe(f, fs.KEYFRAME.FACE_MATERIALS, floor.allFaces())

    # box face data
    boxData = fs.ObjFaceData()
    boxData.saturation = 0.1
    boxData.contrast = 1.6
    boxData.materialID = fs.material("wood")
    box.setFaces(boxData)
    box.insertKeyframe(f, fs.KEYFRAME.FACE_MATERIALS, box.allFaces())

    # Wall face data
    wallData = fs.ObjFaceData()
    wallData.saturation = 0.0
    wallData.contrast = (random.random() - 0.2) * 2 + 4.0
    wallData.uvOffset = [random.random(), random.random()]
    wallData.materialID = fs.material("graybrick")
    wall.setFaces(wallData)
    wall.insertKeyframe(f, fs.KEYFRAME.FACE_MATERIALS, wall.allFaces())

    # Arch face data
    archData = fs.ObjFaceData()
    archData.contrast = (random.random() - 0.2) * 1.1 + 16.0
    archData.uvOffset = [random.random(), random.random()]
    archData.materialID = fs.material("graybrick")
    arch.setFaces(archData)
    arch.insertKeyframe(f, fs.KEYFRAME.FACE_MATERIALS, arch.allFaces())

    # Door face data
    doorData = fs.ObjFaceData()
    offsetX = int(random.random() * 8) / 8.0
    offsetY = int(random.random() * 5) / 5.0
    doorData.uvOffset = [offsetX, offsetY]
    doorData.saturation = 2.0
    doorData.materialID = fs.material("doors")
    door.setFaces(doorData, [0, 2, 6, 8])  # Apply to specific faces
    door.insertKeyframe(f, fs.KEYFRAME.FACE_MATERIALS, [0, 2, 6, 8])

    # StreetLight face data
    streetLightData = fs.ObjFaceData()
    streetLightData.contrast = 1.5
    streetLightData.materialID = fs.material("concrete")
    streetLight.setFaces(streetLightData)  # Apply to all faces
